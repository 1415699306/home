    ...
    public function run()
    {
        ...
        foreach($model as $key=>$val){
            ...
            if($key < 3){
                ...
                $html .= CHtml::tag('li').CHtml::tag ('span').($key+1).CHtml::closeTag('span').CHtml::link(CHtml::encode($val['title']),$owner->createUrl('/'.$owner->module->id.'/list/view',array('id'=>$val['id'])),array('img_url'=>$img,'title'=>$val['title'])).CHtml::closeTag('li');
        }
        $html .='</ul></div>';
        echo $html;
    }
