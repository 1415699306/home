...
class IPbulicCommant extends CWidget{
        ...
        private function _registerCss()
        {
            Yii::app()->getClientScript()->registerCss("#Commant","
                #PublicComment h3{margin-top: 10px;width:105px;height:105px;float:left;background:url('/images/view_icon.png') no-repeat;background-position:0 -223px;}
                ...
                .commant_quote textarea {margin:10px 0 10px 10px;overflow: hidden;border: 1px solid #2C2C2C;width: 86.2%;height: 100px;border-radius: 5px;float: left;border: 1px solid #ccc;border: 2px solid #d4d9dd;-webkit-box-shadow: inset 0 3px 3px #ebebeb;box-shadow: inset 0 3px 3px #ebebeb;}
            ...
            ($zhiz == '1') ? $color_z ="class='top_one'": $color_z = "class='top_tow'";
             foreach($row as $keys)
             {
                 ...
                 if($keys->parent_id ==0){
                    ...
                    .CHtml::closeTag('span')
                    .CHtml::image($keys->userProfile->avatar_min,$keys->username,array('class'=>'quote_user'))
                    ...
                    .CHtml::tag('p').CHtml::tag('span').$this->_checkUser($keys->id)
                    .CHtml::closeTag('span')
                    .CHtml::image($keys->userProfile->avatar_min,$keys->username)
                    .CHtml::tag('em').'<b>'.$keys->username.'</b>:'.date('Y-m-d',$keys->ctime).CHtml::closeTag('em').'<br />'
        ...
        private function _checkUser($id)
        {
            if(Yii::app()->user->isGuest)
                    return CHtml::link('登录','/login/login',array('class'=>'login'));
            else
                return CHtml::link('回复','javascript:post('.$id.')',array('id'=>'post'));
        }
