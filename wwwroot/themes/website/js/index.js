 jQuery(function($){
    new lazy($('.slideItem a'),'img_url','140','180');
    new lazy($('.picshow li a'),'img_url','140','79');
    ...
    new lazy($('.member_bar  a'),'img_url','310','110');
    new lazy($('.member_btn  a'),'img_url','60','47');
    new lazy($('.floatLeft  a'),'img_url','100','100');
    new lazy($('.title  a'),'img_url','228','29');
    new lazy($('.meetingContent  a'),'img_url','280','100');
    new lazy($('.wrapLeft  a'),'img_url','907','85');
    new lazy($('.wrapRight  a'),'img_url','306','85');
    new lazy($('.mainTCenter_TopPic  a'),'img_url','180','145');
    new lazy($('.mainTCenter_list  a'),'img_url','180','146');
    new lazy($('.mainTRight_Tab  a'),'img_url','123','123');
    new lazy($('.centerside_list li a'),'img_url','180','140');
    new lazy($('.sidebar_list_1 li a'),'img_url','145','82');
    new lazy($('.piclist_position  a'),'img_url','160','261');
    new lazy($('.piclist_position_2  a'),'img_url','150','130');
    new lazy($('.centerside_pic_list li a'),'img_url','285','248');
    new lazy($('.rightpic_list  a'),'img_url','251','112');
    new lazy($('.toppic  a'),'img_url','308','420');
    new lazy($('.bottompic  a'),'img_url','155','150');
    new lazy($('.brandList li a'),'img_url','125','73');
    new lazy($('.inbox dt a'),'img_url','80','80');
    new lazy($('.piclist h2 a'),'img_url','160','160'); 
});
...
$(function (){
	...
	(function(){
		...
		var D_val = 152;
		var tickNum ;
		var $view = $("#J-ppg-view-left .view-mid .wrap");//console.log($view.find("ul li:eq(0)").outerWidth())
		...
		$btnLeft.click(function(){//console.log(tick)
			tickNum = $view.find("ul:visible li").size()/2-2
			if (tick <= 0){return}
			tick--;
			$view.animate({left:tick*D_val*-1})	;
		...
		$("#ppg-title-tab span").mouseenter(function(){//console.log($view)
				tick = 0;
				$("#ppg-view-wrap").css("left",0)
				var $ulView = $("#J-ppg-view-wrap ul");//console.log(Math.round(li.size()/2))
				var index = $(this).index();
 ...
 */
$(function(){
	countDown("2013/12/10 23:59:59",".last #day",".last #hour",".last #minute");
});
...
$(function(){
		$("#bd .slideshow .li_2:not(:first)").hide();
			setInterval(function(){
				if($("#bd .slideshow .li_2:last").is(":visible")){
					$("#bd .slideshow .li_2:first").fadeIn("6000");
					$("#bd .slideshow .li_2:last").hide();
				}else{
					$("#bd .slideshow .li_2:visible").next().fadeIn("6000");
					$("#bd .slideshow .li_2:not(:animated)").hide();
				}
			},6000);				
});
