...
<div class="form">
    ...
    <div class="content target">
        ...
        <div class="row">
            ...
            <?php echo $form->textField($model,'video',array('size'=>60,'maxlength'=>1024)); ?>
    ...
    <div class="content target">
        ...
        <div class="row">
            <?php echo $form->labelEx($model,'guests'); ?>
            <?php echo $form->textField($model,'guests',array('size'=>60,'maxlength'=>255)); ?>
            <?php echo $form->error($model,'guests'); ?>
        </div>
        <div class="row">
        <?php echo $form->labelEx($model,'interview_time'); ?>
        <?php echo $form->textField($model,'interview_time',array('class'=>'Wdate','size'=>20,'maxlength'=>80,'onclick'=>"WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d'})",'value'=>!$model->isNewRecord ? date('Y-m-d',$model->interview_time) : '')); ?>
        <?php echo $form->error($model,'interview_time'); ?>
        </div>
        <div class="row">
            <?php echo $form->labelEx($model,'recommand'); ?>
            <?php echo $form->RadioButtonList($model,'recommand',array('1'=>'是','0'=>'否'),array('separator'=>'','template'=>'<li>{input}{label}</li>')); ?>
            <?php echo $form->error($model,'recommand'); ?>
        </div>
        <div class="row">
            <?php echo $form->labelEx($model,'interview_recommand'); ?>
            <?php echo $form->RadioButtonList($model,'interview_recommand',array('1'=>'是','0'=>'否'),array('separator'=>'','template'=>'<li>{input}{label}</li>')); ?>
            <?php echo $form->error($model,'interview_recommand'); ?>
        </div>
        <div class="row">
            <?php echo $form->labelEx($model,'channel_recommand'); ?>
            <?php echo $form->RadioButtonList($model,'channel_recommand',array('1'=>'是','0'=>'否'),array('separator'=>'','template'=>'<li>{input}{label}</li>')); ?>
    ...
    <div class="content target">
        <h5>文章内容</h5>
        <div class="row">
            <?php echo $form->labelEx($model->celebrityContent,'content'); ?>
            <?php $this->widget('ext.widgets.baiduEdit.IBaiduEdit',array('model'=>$model->celebrityContent,'form'=>$form,'attribute'=>'content','value'=>$newContent));?>
            <?php echo $form->error($model->celebrityContent,'content'); ?>
        </div> 
    </div>
    <div class="content">
        <h5>缩略图设置</h5>
        <div class="row thumb_body">
            <?php $this->widget('ext.widgets.cut.ICutThumb',array('app_id'=>BaseApp::CELEBRITY,'model'=>$model,'folder'=>'celebrity','ext'=>'thumb','thumb'=>!empty($model->thumbs->track_id) ? $model->thumbs->track_id : null));?>  
        </div>
    </div>
