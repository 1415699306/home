<div class="form">
    ...
    <div class="content target">
        <h5>文章内容</h5>
        <div class="row">
            <?php echo $form->labelEx($model->lifeContent,'content'); ?>
            <?php $this->widget('ext.widgets.baiduEdit.IBaiduEdit',array('model'=>$model->lifeContent,'form'=>$form,'attribute'=>'content','value'=>$newContent));?>
            <?php echo $form->error($model->lifeContent,'content'); ?>
        </div>  
    </div>
    <div class="content">
        <h5>缩略图设置</h5>
        <div class="row thumb_body">
            <?php $this->widget('ext.widgets.cut.ICutThumb',array('app_id'=>BaseApp::LIFE,'model'=>$model,'folder'=>'life','ext'=>'thumb','thumb'=>!empty($model->thumbs->track_id) ? $model->thumbs->track_id : null));?>  
        </div>
