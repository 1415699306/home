<div class="form">
    ...
    <div class="content target">
        ...
        <div class="row">
            ...
            <?php echo $form->RadioButtonList($model,'recommend',array('0'=>'否','1'=>'是'),array('separator'=>'','template'=>'<li>{input}{label}</li>')); ?>
    ...
    <div class="content target">
        <h5>文章内容</h5>
        <div class="row">
            <?php echo $form->labelEx($model->articleContent,'content'); ?>
            <?php $this->widget('ext.widgets.baiduEdit.IBaiduEdit',array('model'=>$model->articleContent,'form'=>$form,'attribute'=>'content','value'=>$newContent));?>
            <?php echo $form->error($model->articleContent,'content'); ?>
        </div>  
    </div>
    <div class="content">
        <h5>缩略图设置</h5>
        <div class="row thumb_body">
            <?php $this->widget('ext.widgets.cut.ICutThumb',array('app_id'=>BaseApp::ARTICLE,'model'=>$model,'folder'=>'article','ext'=>'thumb','thumb'=>!empty($model->thumbs->track_id) ? $model->thumbs->track_id : null));?>  
        </div>
    </div>
