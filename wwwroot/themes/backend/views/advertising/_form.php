...
<div class="row thumb_body">
    <?php $this->widget('ext.widgets.cut.ICutThumbByOne',array('app_id'=>BaseApp::ADVERTISING,'model'=>$model,'folder'=>'advertising','ext'=>'thumb','thumb'=>!empty($model->thumbs->track_id) ? $model->thumbs->track_id : null));?>  
</div>
</div>
...
var len = $('.image_list img').length;
if(0 < len){
    $('.thumb_body').css('display','block');
    document.getElementById("Advertising_type_0").disabled = true;
}
if($('#Advertising_type_1').attr('checked','checked')){
    $('.thumb_body').css('display','block');
}
$('#Advertising_type_0').change(function(){
    $('.thumb_body').css('display','none');
});
$('#Advertising_type_1').change(function(){
    $('.thumb_body').css('display','block');
});
</script>
<style>
