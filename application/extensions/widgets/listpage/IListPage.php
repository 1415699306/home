...
class IListPage extends CWidget{
    ...
    public function _registerHtml()
    {
        ...
        $nextModel=$className::model()->dbConnection->createCommand("select id,title from `{$table}` where id > :id and status=:status order by id asc limit 1")->bindValues(array(':id'=>$id,':status'=>$this->status))->queryRow();
        $prevModel=$className::model()->dbConnection->createCommand("select id,title from `{$table}` where id < :id and status=:status order by id desc limit 1")->bindValues(array(':id'=>$id,':status'=>$this->status))->queryRow();
