...
class INavLevel extends CWidget{
    ...
    private function _Universal($category,$owner)
    {
        ...
        if(isset($owner->category_menu) && is_array($owner->category_menu)){
            foreach($owner->category_menu as $key=>$val){
                $html.= CHtml::link($key,$val,array('target'=>'_blank','style'=>'border:none;border-left:1px solid #434a55;'));
            }
        }
