...
class FileComponent extends CComponent{
    ...
    */
    ...
    public function createRoute($ext = null)
    {
        ...
        $root_path = UPLOSD_PATH.DIRECTORY_SEPARATOR.self::ATTACHMENT;
        if(!is_dir($root_path))mkdir($root_path,0777);
        $folder = self::ATTACHMENT.DIRECTORY_SEPARATOR.$this->route;
        if($ext == null)
        {
            $path = UPLOSD_PATH.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$year.DIRECTORY_SEPARATOR.$month.DIRECTORY_SEPARATOR;
            $route = $year.DIRECTORY_SEPARATOR.$month.DIRECTORY_SEPARATOR;
        }
        else
        {
            $path = UPLOSD_PATH.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$ext.DIRECTORY_SEPARATOR.$year.DIRECTORY_SEPARATOR.$month.DIRECTORY_SEPARATOR;
            $route = $ext.DIRECTORY_SEPARATOR.$year.DIRECTORY_SEPARATOR.$month.DIRECTORY_SEPARATOR;
        }
        //如果目录不存在就创建目录 
        try 
        {
            if(!is_dir(UPLOSD_PATH.DIRECTORY_SEPARATOR.$folder)){
                 mkdir(UPLOSD_PATH.DIRECTORY_SEPARATOR.$folder,0777);    
            }
            if(!is_null($ext)){
                $folder = UPLOSD_PATH.DIRECTORY_SEPARATOR.$folder.DIRECTORY_SEPARATOR.$ext;
                if(!is_dir($folder)){
                    mkdir($folder,0777);    
                }                
            }
